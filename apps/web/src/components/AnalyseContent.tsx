...
type CategorieEquipement = "conformes" | "degrades" | "manquants" | "nonTrouves" | "horsContrat";

const SECTION_LABEL: Record<CategorieEquipement, string> = {
  conformes: "Bon état",
  degrades: "État dégradé",
  manquants: "Manquants",
  nonTrouves: "Non trouvés",
...
export function AnalyseContent({ contratId: id, pecId, rapport }: AnalyseContentProps) {
  const {
    contrat,
    client,
    site,
  ...
  function StatCard({
    ...
    value: number;
    color?: string;
    filtreCible?: CategorieEquipement;
  }) {
    const isActive = filtreCible && filtre === filtreCible;
    const content = (
      <>
    ...
    return (
      <button
        type="button"
        onClick={() => toggleFiltre(filtreCible)}
        className={`rounded-lg border p-4 text-left transition hover:border-indigo-300 hover:shadow-sm ${
          isActive ? "border-indigo-600 ring-1 ring-indigo-600" : "border-slate-200"
        } bg-white`}
      >
        {content}
      </button>
  ...
  const showSection = (categorie: CategorieEquipement) => filtre === null || filtre === categorie;

  return (
    <div className="flex flex-col gap-6">
      <ContratNav contratId={id} pecId={pecId} active="analyse" />
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <StatCard label="Bon état" value={stats.nbConformes} color="text-emerald-600" filtreCible="conformes" />
        <StatCard label="État dégradé" value={stats.nbDegrades} color="text-amber-600" filtreCible="degrades" />
        <StatCard label="Manquants" value={stats.nbManquants} color="text-red-600" filtreCible="manquants" />
        <StatCard label="Non trouvés" value={stats.nbNonTrouves} color="text-red-600" filtreCible="nonTrouves" />
        <StatCard label="Hors contrat" value={stats.nbHorsContrat} color="text-indigo-600" filtreCible="horsContrat" />
      ...
      {filtre && (
        <div className="flex items-center justify-between rounded-md border border-indigo-100 bg-indigo-50/60 px-4 py-2 text-sm text-indigo-700">
          <span>
            Filtré sur : <strong>{SECTION_LABEL[filtre]}</strong>
          </span>
          <button type="button" onClick={() => setFiltre(null)} className="font-medium text-indigo-600 hover:text-indigo-900">
            Voir tout
          </button>
        </div>
      )}
      ...
      {manquants.length > 0 && showSection("manquants") && (
        <section>
      ...
      {nonTrouves.length > 0 && showSection("nonTrouves") && (
        <section>
          <h2 className="mb-2 text-sm font-semibold text-red-700">
      ...
      {degrades.length > 0 && showSection("degrades") && (
        <section>
      ...
      {horsContrat.length > 0 && showSection("horsContrat") && (
        <section>
      ...
      {conformes.length > 0 && showSection("conformes") && (
        <section>
