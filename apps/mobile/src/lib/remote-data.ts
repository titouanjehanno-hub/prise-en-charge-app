// Couche d'accès directe à Supabase (jamais utilisée hors-ligne). Utilisée par
// la couche publique (data.ts, cache-first / hors-ligne) et par le moteur de
// synchro (sync-engine.ts) pour rejouer les écritures en attente.
...
export async function creerNouvellePriseEnCharge(contrat: Contrat, id?: string): Promise<PriseEnCharge> {
  const { data: userData } = await supabase.auth.getUser();
  const userId = userData.user?.id;
  if (!userId) throw new Error("Utilisateur non connecté.");
...
// Toujours un upsert par id (fourni par l'appelant, généré côté client) :
// fonctionne aussi bien pour la création que la mise à jour, en ligne comme
// pour rejouer une écriture faite hors-ligne.
...
export async function deletePhoto(photo: { id: string; storagePath: string }): Promise<void> {
